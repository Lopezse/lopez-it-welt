...
class AntiRuleBreakTester {
    ...
    async testSystemTimeValidation() {
        ...
        try {
            ...
            if (result.valid) {
                ...
                this.testResults.push({ test: 'System-Zeit', status: 'FAILED', reason: result.reason });
            }
        } catch (error) {
            console.log('   ❌ System-Zeit Test fehlgeschlagen:', error.message);
            this.testResults.push({ test: 'System-Zeit', status: 'ERROR', reason: error.message });
        }
        console.log('');
    }
        ...
        for (const testCase of testCases) {
            try {
                ...
                if (result.valid === testCase.expected) {
                    console.log(`   ✅ "${testCase.action}": ${result.valid ? 'ERLAUBT' : 'BLOCKIERT'}`);
                } else {
                    console.log(`   ❌ "${testCase.action}": Erwartet ${testCase.expected}, aber ${result.valid}`);
                ...
                this.testResults.push({ test: 'Datumskopieren', status: 'ERROR', reason: error.message });
                return;
            }
        }
    ...
    async testApprovalSystem() {
        ...
        try {
            ...
            // Test ohne Freigabe
            const result1 = await this.hook.validateBeforeAction('Test ohne Freigabe');
            if (!result1.valid) {
                console.log('   ✅ Aktion ohne Freigabe blockiert');
            } else {
                console.log('   ❌ Aktion ohne Freigabe nicht blockiert');
                this.testResults.push({ test: 'Freigabe-System', status: 'FAILED', reason: 'Aktion nicht blockiert' });
                return;
            }
            ...
            if (result2.valid) {
                ...
                this.testResults.push({ test: 'Freigabe-System', status: 'FAILED', reason: 'Aktion blockiert trotz Freigabe' });
                return;
            }
            ...
            this.testResults.push({ test: 'Freigabe-System', status: 'ERROR', reason: error.message });
        }
        console.log('');
    }
    ...
    async testTimeTracking() {
        ...
        try {
            ...
            if (result.valid) {
                ...
                this.testResults.push({ test: 'Zeiterfassung', status: 'FAILED', reason: result.reason });
            }
        } catch (error) {
            console.log('   ❌ Zeiterfassung Test fehlgeschlagen:', error.message);
            this.testResults.push({ test: 'Zeiterfassung', status: 'ERROR', reason: error.message });
        }
        console.log('');
    }
