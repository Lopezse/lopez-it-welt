...
async function startNewSession() {
  ...
  console.log(
    `🎯 Kategorie: ${CONFIG.categories.find(c => c.value === category)?.label}`
  );
  console.log(
    `🔥 Priorität: ${CONFIG.priorities.find(p => p.value === priority)?.label}`
  );
  console.log(`📝 Beschreibung: ${session.description}`);
  console.log(
    `⏰ Start: ${new Date(session.start_time).toLocaleString('de-DE')}`
  );
}
...
function showReport() {
  ...
  Object.entries(categoryStats).forEach(([category, minutes]) => {
    ...
    const categoryLabel =
      CONFIG.categories.find(c => c.value === category)?.label || category;
  ...
  Object.entries(priorityStats).forEach(([priority, minutes]) => {
    ...
    const mins = minutes % 60;
    const priorityLabel =
      CONFIG.priorities.find(p => p.value === priority)?.label || priority;
  ...
  recentSessions.forEach(session => {
    ...
    const categoryLabel =
      CONFIG.categories.find(c => c.value === session.category)?.label ||
      session.category;

    console.log(
      `   ${session.module} (${categoryLabel}) - ${duration} - ${startTime}`
    );
  });
}
