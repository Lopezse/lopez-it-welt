...
class MemorySystemIntegration {
    ...
    async testComplianceCheck() {
        ...
        try {
           ...
           LIMIT 5`,
                ...
                log(`${status} ${test.action}: ${result}`, hasViolations === test.expected ? 'green' : 'red');
            }
    ...
    async showStatistics() {
        ...
        try {
            ...
            // Aktive Regeln
            const [activeRules] = await this.connection.execute('SELECT COUNT(*) as active FROM ki_rules WHERE is_active = TRUE');
