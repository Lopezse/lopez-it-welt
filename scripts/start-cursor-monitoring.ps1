...
function Show-SystemStatus {
    Write-Host ""
    Write-Host "🛡️ CURSOR-MONITORING STATUS:" -ForegroundColor Cyan
...
function Test-System {
    Write-Host "🧪 System-Test wird ausgeführt..." -ForegroundColor Yellow
    
    # Test 1: Node.js verfügbar
    try {
        $nodeVersion = node --version
        Write-Host "✅ Node.js verfügbar: $nodeVersion" -ForegroundColor Green
    }
    catch {
        Write-Host "❌ Node.js nicht verfügbar" -ForegroundColor Red
        return $false
    }
...
if (!$AutoStart -and !$ShowStatus -and !$Test -and !$Force) {
    Write-Host "ℹ️ Keine Parameter angegeben. Verwenden Sie:" -ForegroundColor Yellow
    ...
    Write-Host "   -Test : Führt einen System-Test durch" -ForegroundColor White
