...
class AgentenAktivator {
  ...
  sendeConsoleBenachrichtigung(status, offeneAufgaben) {
    // Sicherstellen, dass offeneAufgaben ein Array ist
    const aufgabenArrayAktivator = Array.isArray(offeneAufgaben) ? offeneAufgaben : [];
    ...
    console.log(`📋 Offene Aufgaben: ${aufgabenArrayAktivator.length}`);

    if (aufgabenArrayAktivator.length > 0) {
      console.log("\n⚠️  OFFENE AUFGABEN:");
      aufgabenArrayAktivator.forEach((aufgabe, index) => {
        console.log(`${index + 1}. [${aufgabe.prioritaet.toUpperCase()}] ${aufgabe.beschreibung}`);
      });
    }

    console.log("\n🎯 EMPFOHLENE NÄCHSTE SCHRITTE:");
    if (status.naechsteSchritte && Array.isArray(status.naechsteSchritte)) {
  ...
  starteUeberwachung() {
    ...
    console.log(
      `🔄 Starte kontinuierliche Überwachung (Intervall: ${konfiguration.ueberwachungsIntervall / 1000}s)`,
    );
    ...
    setInterval(async () => {
      ...
      // Sicherstellen, dass offeneAufgaben ein Array ist
      if (offeneAufgaben && Array.isArray(offeneAufgaben) && offeneAufgaben.length > 0) {
