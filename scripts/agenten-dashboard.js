...
class AgentenDashboard {
  ...
  generierePerformanceDaten(gedaechtnis) {
    ...
    return {
      ...
      groessteDatei: dateien.reduce((max, datei) => (datei.groesse > max.groesse ? datei : max), {
        groesse: 0,
      }),
      neuesteDatei: dateien.reduce(
        (neueste, datei) =>
          new Date(datei.letzteAenderung) > new Date(neueste.letzteAenderung) ? datei : neueste,
