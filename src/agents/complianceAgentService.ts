...
const validateApiKey = (req: Request, res: Response, next: NextFunction): void => {
  ...
  const expectedKey = process.env.AGENT_API_KEY || "default-key-change-in-production";
...
app.post("/quality/check", validateApiKey, async (req: Request, res: Response): Promise<void> => {
  try {
    const { input, context, options } = req.body;
...
app.get("/memory/stats", validateApiKey, async (req: Request, res: Response): Promise<void> => {
  try {
...
app.post("/batch/process", validateApiKey, async (req: Request, res: Response): Promise<void> => {
  try {
