/**
 * GoBD Reports Component - Enterprise++ Standard E.2.2
 * 
 * GoBD-Compliance-Berichte generieren und exportieren
 */
...
interface GoBDReportsProps {
  onReportGenerated?: (reportType: string) => void;
}

export function GoBDReports({ onReportGenerated }: GoBDReportsProps) {
  ...
  const generateReport = async (reportType: string, format: "pdf" | "csv", timeRange: string) => {
    try {
      ...
      const response = await fetch("/api/compliance/gobd/reports/generate", {
        method: "POST",
      ...
      logger.error("Fehler beim Generieren des GoBD-Berichts", err, { reportType, format, timeRange });
  ...
  const reportTypes = [
    { id: "compliance", label: "Compliance-Bericht", description: "Vollständiger GoBD-Compliance-Status" },
    { id: "invoices", label: "Rechnungs-Verifikation", description: "Hash-Verifikation für alle Rechnungen" },
    { id: "backups", label: "Backup-Verifikation", description: "Hash-Verifikation für alle Backups" },
    { id: "audit-trail", label: "Audit-Trail-Bericht", description: "Vollständiger Audit-Trail für GoBD" },
  ];
  ...
  return (
    <Card className="dark:bg-gray-800 dark:border-gray-700">
      <div className="p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">GoBD-Berichte</h3>
