/**
 * GoBD Reports Generate API - Enterprise++ Standard E.2.2
 * 
 * POST /api/compliance/gobd/reports/generate - GoBD-Berichte generieren
 * 
...
export async function POST(request: NextRequest) {
  try {
    ...
    const reportId = `gobd-report-${Date.now()}`;
    const downloadUrl = `/api/compliance/gobd/reports/download/${reportId}`;
    ...
    logger.error("Fehler beim Generieren des GoBD-Berichts", error);
    return NextResponse.json(
