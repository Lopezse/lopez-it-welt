/**
 * Orchestrator Workflow Pause API - Enterprise++ Standard
 * 
 * POST /api/orchestrator/workflows/[id]/pause - Workflow pausieren
 * 
...
export async function POST(
    ...
    try {
        ...
        if (executions.length === 0) {
            return NextResponse.json(
                { success: false, message: "Keine aktive Execution gefunden" },
                { status: 404 }
            );
        }
        ...
        return NextResponse.json(
            { 
                success: false, 
                message: error instanceof Error ? error.message : "Fehler beim Pausieren des Workflows" 
            },
            { status: 500 }
        );
    }
