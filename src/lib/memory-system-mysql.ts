...
export class MySQLMemorySystem {
  ...
  private validateRule(rule: KIRule): void {
    if (!rule.rule_text || !rule.category) {
      throw new Error('Ungültige Regel: rule_text und category sind erforderlich');
    }
  }
  ...
  async getMemorySessionsByContext(context: string): Promise<KIMemorySession[]> {
    this.validateConnection();
  ...
  async checkCompliance(action: string, context: string): Promise<ComplianceResult> {
    this.validateConnection();

    try {
      // Relevante Regeln finden
      const relevantRules = await this.searchRulesByContext(action + ' ' + context);

      // Compliance-Status prüfen
      const complianceChecks = await Promise.all(
        relevantRules.map(async (rule) => {
          const isCompliant = await this.validateRuleCompliance(rule, action, context);
          return { rule, isCompliant };
        })
      );

      const compliantRules = complianceChecks.filter(check => check.isCompliant).map(check => check.rule);
      const violations = complianceChecks.filter(check => !check.isCompliant).map(check => check.rule.rule_text);

      const score = relevantRules.length > 0 ? (compliantRules.length / relevantRules.length) * 100 : 100;
  ...
  private async validateRuleCompliance(rule: KIRule, action: string, context: string): Promise<boolean> {
    ...
    if (rule.category === 'compliance') {
      if (ruleLower.includes('dsgvo') && actionLower.includes('formular')) {
        return actionLower.includes('consent') || actionLower.includes('datenschutz');
      }
      if (ruleLower.includes('cookie') && actionLower.includes('cookie')) {
        return actionLower.includes('consent') || actionLower.includes('opt-out');
      }
    }
    ...
    if (rule.category === 'enterprise') {
      ...
      if (ruleLower.includes('lint') && actionLower.includes('code')) {
        return actionLower.includes('eslint') || actionLower.includes('prettier');
      }
    }
  ...
  async loadDefaultRules(): Promise<void> {
    ...
    try {
      ...
      const dsgvoRules: KIRule[] = [
        {
          rule_text: 'Alle Kontaktformulare müssen DSGVO-konform sein mit Consent-Checkbox und Datenschutzerklärung',
          category: 'compliance',
          tags: ['dsgvo', 'formular', 'consent'],
          priority: 'kritisch'
        },
        {
          rule_text: 'Personenbezogene Daten dürfen nur mit expliziter Einwilligung verarbeitet werden',
          category: 'compliance',
  ...
  async validateAgentAction(agentType: string, action: string, context: string): Promise<ComplianceResult> {
    ...
    if (agentType === 'compliance') {
      ...
      if (action.toLowerCase().includes('formular') && !action.toLowerCase().includes('consent')) {
        complianceResult.is_compliant = false;
        complianceResult.violations.push('DSGVO: Consent-Checkbox erforderlich');
      }
    }

    if (agentType === 'enterprise') {
      // Enterprise++-spezifische Prüfungen
      if (action.toLowerCase().includes('test') && !action.toLowerCase().includes('coverage')) {
        complianceResult.is_compliant = false;
        complianceResult.violations.push('Enterprise++: Test-Coverage erforderlich');
      }
    }
