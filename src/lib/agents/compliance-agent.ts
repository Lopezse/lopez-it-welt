...
export class ComplianceAgent extends KIAgent {
    ...
    private async validateDSGVOCompliance(formData: DSGVOFormData, rules: string[]): Promise<ComplianceResult> {
    ...
    private async generateDSGVOCompliantForm(formData: DSGVOFormData, rules: string[]): Promise<string> {
...
export default ${formData.formName};
    ...
    private determineFieldType(fieldName: string): string {
        ...
        if (lowerField.includes('message') || lowerField.includes('nachricht') || lowerField.includes('text')) return 'textarea';
