...
// =====================================================
// MYSQL TYPEN - Enterprise++ Type Safety
// =====================================================

/**
 * Typ-Definitionen für MySQL-Query-Ergebnisse
 * Verwendet für vollständige Typensicherheit bei Datenbank-Zugriffen
 */
export type QueryResult<T = RowDataPacket> = [T[], mysql.FieldPacket[]];
export type InsertResult = [ResultSetHeader, mysql.FieldPacket[]];
export type UpdateResult = [ResultSetHeader, mysql.FieldPacket[]];
export type DeleteResult = [ResultSetHeader, mysql.FieldPacket[]];

/**
 * Type-Guard für RowDataPacket-Arrays
 */
export function isRowDataPacketArray(
  result: RowDataPacket[] | OkPacket | ResultSetHeader | RowDataPacket[],
): result is RowDataPacket[] {
  return Array.isArray(result) && result.length > 0 && typeof result[0] === "object" && "constructor" in result[0];
}

/**
 * Type-Guard für ResultSetHeader (INSERT/UPDATE/DELETE)
 */
export function isResultSetHeader(
  ...
  return (
    ...
    result !== null &&
    ("insertId" in result || "affectedRows" in result)
  );
}
...
const dbConfig = {
  ...
  database: process.env.DB_NAME || "lopez_it_welt", // XAMPP: Standard-Datenbankname
  waitForConnections: true,
  connectionLimit: parseInt(process.env.DB_CONNECTION_LIMIT || "10"), // XAMPP: Reduziert für lokale Entwicklung
  queueLimit: 0,
  multipleStatements: false,
  // XAMPP-spezifische Einstellungen
  charset: "utf8mb4",
  timezone: "+00:00", // UTC für Konsistenz
};
...
export async function initializeDatabase(): Promise<void> {
  ...
  if (currentDbName === prodDbName) {
    ...
    console.error("🛑 =====================================================");
    throw new Error(
      "SICHERHEITSSTOPP: initializeDatabase() wurde blockiert. " +
      `Datenbank "${currentDbName}" ist als PROD markiert. ` +
  ...
  try {
    ...
    const [existingTables] = await connection.execute<RowDataPacket[]>(`
      SELECT COUNT(*) as count FROM information_schema.TABLES 
      WHERE TABLE_SCHEMA = DATABASE()
    `);
    const tableCount = (existingTables as any[])[0]?.count || 0;
    ...
    try {
      ...
      if (tables.length > 0) {
        ...
        const [allColumns] = await connection.execute<RowDataPacket[]>(
          "SHOW COLUMNS FROM lopez_customers"
        );
        
        const existingColumns = allColumns.map((col: any) => col.Field);
        ...
        if (missingColumns.length > 0) {
          ...
          const [idColumns] = await connection.execute<RowDataPacket[]>(
    ...
    const passwordPolicyItems = [
        { key: 'min_length', label: 'Mindestlänge (Zeichen)', description: 'Enterprise++ Standard: 12 Zeichen', type: 'number', default_value: '12', sort_order: 1 },
        { key: 'max_length', label: 'Maximale Länge (Zeichen)', description: 'Empfohlen: 128 Zeichen', type: 'number', default_value: '128', sort_order: 2 },
        { key: 'require_uppercase', label: 'Großbuchstaben (A-Z) erforderlich', description: 'Mindestens ein Großbuchstabe', type: 'boolean', default_value: 'true', sort_order: 3 },
        { key: 'require_lowercase', label: 'Kleinbuchstaben (a-z) erforderlich', description: 'Mindestens ein Kleinbuchstabe', type: 'boolean', default_value: 'true', sort_order: 4 },
        { key: 'require_numbers', label: 'Zahlen (0-9) erforderlich', description: 'Mindestens eine Zahl', type: 'boolean', default_value: 'true', sort_order: 5 },
        { key: 'require_special_chars', label: 'Sonderzeichen (!@#$%^&*) erforderlich', description: 'Mindestens ein Sonderzeichen', type: 'boolean', default_value: 'true', sort_order: 6 },
        { key: 'expiration_days', label: 'Passwort-Ablauf (Tage)', description: '0 = Kein Ablauf', type: 'number', default_value: '180', sort_order: 7 },
        { key: 'prevent_reuse', label: 'Verbot alter Passwörter (Historie)', description: 'Verhindert Wiederverwendung der letzten 5 Passwörter', type: 'boolean', default_value: 'true', sort_order: 8 },
        { key: 'max_failed_attempts', label: 'Maximale Fehlversuche', description: 'Brute-Force-Schutz', type: 'number', default_value: '5', sort_order: 9 },
        { key: 'lockout_duration', label: 'Sperrdauer (Minuten)', description: 'Dauer der Kontosperre nach Fehlversuchen', type: 'number', default_value: '15', sort_order: 10 },
    ];
    ...
    for (const item of passwordPolicyItems) {
        ...
        `, [item.key, item.label, item.description, item.type, item.default_value, item.sort_order]);
    }
