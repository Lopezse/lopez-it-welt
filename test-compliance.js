...
const { ComplianceScannerMySQL } = require("./src/lib/compliance-system-mysql.ts");
...
async function testComplianceSystem() {
  ...
  try {
    ...
    if (result.results.dsgvoCompliance) {
      ...
      console.log(`✅ Bestandene Checks: ${compliance.passedChecks}/${compliance.totalChecks}`);

      console.log("\n📋 Details:");
      Object.entries(compliance.details).forEach(([key, detail]) => {
        const status = detail.found ? "✅" : "❌";
        const score = detail.score || 0;
        console.log(`${status} ${key}: ${score}% ${detail.path ? `(${detail.path})` : ""}`);
      });
