...
async function GET(request) {
    try {
        ...
        const hasViewPermission = validation.session?.permissions.some((p)=>p === "admin.projects.view" || p.startsWith("admin.projects."));
...
async function POST(request) {
    try {
        ...
        const hasCreatePermission = validation.session?.permissions.some((p)=>p === "admin.projects.create" || p === "admin.projects.edit");
        ...
        const { project_name, description, customer_id, status = "planned", priority = "normal", start_date, end_date, budget_amount, assigned_user_id } = body;
        ...
        const [countResult] = await pool.execute("SELECT COUNT(*) as count FROM lopez_projects WHERE DATE(created_at) = CURDATE()");
        const count = countResult[0]?.count || 0;
        const projectCode = `P-${dateStr}-${String(count + 1).padStart(3, "0")}`;
        const [result] = await pool.execute(`INSERT INTO lopez_projects 
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f452c945._.js.map