module.exports = [
"[project]/.next-internal/server/app/api/admin/ai/costs/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/src/app/api/admin/ai/costs/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        // Kosten pro Tag
        const [dailyCosts] = await pool.execute(`
      SELECT 
        date,
    ...
    `, [
        ...
        // Kosten pro Provider
        const [providerCosts] = await pool.execute(`
      SELECT 
        provider,
        SUM(tokens_input) as tokens_input,
        SUM(tokens_output) as tokens_output,
    ...
    `, [
        ...
        // Kosten pro Endpoint
        const [endpointCosts] = await pool.execute(`
      SELECT 
        endpoint,
        COUNT(*) as requests,
        SUM(tokens_input) as tokens_input,
    ...
    `, [
        ...
        // Heute
        const today = new Date().toISOString().split("T")[0];
        const todayCost = dailyCosts.find((d)=>new Date(d.date).toISOString().split("T")[0] === today);
        // Gestern
        const yesterday = new Date(Date.now() - 86400000).toISOString().split("T")[0];
        const yesterdayCost = dailyCosts.find((d)=>new Date(d.date).toISOString().split("T")[0] === yesterday);
        // Totals
        const totalCost = dailyCosts.reduce((sum, d)=>sum + Number(d.cost), 0);
        const totalTokensInput = dailyCosts.reduce((sum, d)=>sum + Number(d.tokens_input), 0);
        const totalTokensOutput = dailyCosts.reduce((sum, d)=>sum + Number(d.tokens_output), 0);
        // Limit-Status
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b70210f9._.js.map