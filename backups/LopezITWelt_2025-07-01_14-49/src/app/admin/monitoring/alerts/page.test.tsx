...
const mockUseEffect = jest.fn((callback) => {
    ...
    if (!mockUseEffect.called) {
        callback();
        mockUseEffect.called = true;
    }
});
...
describe('AlertsPage', () => {
    ...
    it('should display alerts list', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Kritischer Sicherheitsvorfall')).toBeInTheDocument();
            expect(screen.getByText('Hohe CPU-Auslastung')).toBeInTheDocument();
            expect(screen.getByText('Festplattenspeicher wird knapp')).toBeInTheDocument();
        });
    });
    ...
    it('should filter alerts when filter button is clicked', async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.queryByText('Netzwerk-Latenz erhöht')).not.toBeInTheDocument();
        });
    });
    ...
    it('should display notification settings', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Benachrichtigungseinstellungen')).toBeInTheDocument();
