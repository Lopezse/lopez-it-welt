...
const Layout: React.FC<LayoutEigenschaften> = ({
  children,
  seitenTitel,
  seitenBeschreibung,
}) => {
      ...
      <div className='min-h-screen flex flex-col bg-gray-50'>
        ...
        <div className='flex flex-1'>
          ...
          <main
            className='flex-1'
            role='main'
            aria-label={seitenTitel || 'Admin-Bereich'}
          >
            {seitenBeschreibung && (
              <div className='sr-only'>{seitenBeschreibung}</div>
            )}
            {children}
          </main>
        </div>
  ...
  return (
    <div className='min-h-screen flex flex-col'>
      ...
      <main
        className='flex-1'
        role='main'
        aria-label={seitenTitel || 'Hauptinhalt'}
      >
        {seitenBeschreibung && (
          <div className='sr-only'>{seitenBeschreibung}</div>
        )}
