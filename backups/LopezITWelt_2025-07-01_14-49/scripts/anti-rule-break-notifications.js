...
class AntiRuleBreakNotifications {
    ...
    showStatus() {
        ...
        console.log(`   Slack: ${this.config.slack.enabled ? '✅ Aktiviert' : '❌ Deaktiviert'}`);
        console.log(`   Telegram: ${this.config.telegram.enabled ? '✅ Aktiviert' : '❌ Deaktiviert'}`);
        console.log(`   E-Mail: ${this.config.email.enabled ? '✅ Aktiviert' : '❌ Deaktiviert'}`);
        console.log(`   Discord: ${this.config.discord.enabled ? '✅ Aktiviert' : '❌ Deaktiviert'}`);
        console.log(`   Gesendete Benachrichtigungen: ${this.notificationHistory.filter(n => n.sent).length}`);
        console.log(`   Fehlgeschlagene Benachrichtigungen: ${this.notificationHistory.filter(n => !n.sent).length}`);
    }
    ...
    saveHistory() {
        ...
        try {
            ...
            console.log(`📝 Benachrichtigungs-Historie in ${historyFile} gespeichert`);
        } catch (error) {
    ...
    switch (command) {
        ...
        case 'history':
            ...
            notifications.notificationHistory.forEach((notification, index) => {
                console.log(`   ${index + 1}. ${notification.timestamp} - ${notification.sent ? '✅' : '❌'} ${notification.violation.rule}`);
            });
            break;
        default:
            console.log('🔔 Anti-Regelbruch Enterprise++ Modul - Monitoring & Benachrichtigungen');
            console.log('Verwendung:');
            console.log('  node anti-rule-break-notifications.js test     - Test-Benachrichtigung senden');
            console.log('  node anti-rule-break-notifications.js status   - Status anzeigen');
            console.log('  node anti-rule-break-notifications.js history  - Historie anzeigen');
    }
} 