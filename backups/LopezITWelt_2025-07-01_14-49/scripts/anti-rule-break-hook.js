...
class AntiRuleBreakHook {
    ...
    async validateBeforeAction(action, targetFile = null) {
        ...
        if (!timeValidation.valid) {
            ...
            return { valid: false, reason: timeValidation.reason, timestamp, rule: 'System-Zeit' };
        }
        ...
        if (!dateValidation.valid) {
            ...
            return { valid: false, reason: dateValidation.reason, timestamp, rule: 'Datumsvalidierung' };
        }
        ...
        if (targetFile && this.isMdFile(targetFile)) {
            ...
            if (!structureValidation.valid) {
                ...
                return { valid: false, reason: structureValidation.reason, timestamp, rule: 'Md-Struktur' };
            }
        }
        ...
        if (!this.approvalGiven && this.config.requireApproval) {
            ...
            return { valid: false, reason: 'Keine Freigabe vorhanden', timestamp, rule: 'Freigabe-Erfordernis' };
        }
        ...
        if (!timeTrackingValidation.valid) {
            ...
            return { valid: false, reason: timeTrackingValidation.reason, timestamp, rule: 'Zeiterfassung' };
        }
    ...
    async validateSystemTime() {
        try {
            ...
            if (currentTime < minValidDate) {
                return { valid: false, reason: 'System-Zeit ist in der Vergangenheit', timestamp };
            }

            return { valid: true, timestamp };
        } catch (error) {
            return { valid: false, reason: 'System-Zeit-Abfrage fehlgeschlagen', timestamp: new Date().toISOString() };
        }
    }
    ...
    async validateNoDateCopying(action) {
        ...
        for (const blockedDate of blockedDates) {
            if (action.includes(blockedDate)) {
                return { valid: false, reason: `Datumskopieren erkannt: ${blockedDate}`, timestamp };
            }
        }
    ...
    try {
        ...
        for (const file of changedFiles) {
            if (hook.isMdFile(file)) {
                const validation = hook.validateBeforeAction(`Änderung in ${file}`, file);
                if (!validation.valid) {
                    console.error(`❌ Pre-Commit Hook blockiert: ${validation.reason}`);
                    process.exit(1);
                }
            }
        }
