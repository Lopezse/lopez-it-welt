...
import { CompleteSessionData, CreateSessionData, TimeSession } from "./time-tracking-types";
...
export class TimeTrackingServerService {
  ...
  static getStats() {
    ...
    sessions.forEach((session) => {
      ...
      if (session.duration_minutes) {
        moduleStats[session.module] = (moduleStats[session.module] || 0) + session.duration_minutes;
      }
  ...
  static updateSession(id: number, updateData: Partial<TimeSession>): TimeSession | null {
    const sessions = loadSessions();
