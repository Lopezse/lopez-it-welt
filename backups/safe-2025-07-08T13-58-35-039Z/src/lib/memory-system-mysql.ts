...
export class MySQLMemorySystem {
  ...
  async checkCompliance(action: string, context: string): Promise<ComplianceResult> {
    ...
    try {
      ...
      const score =
        relevantRules.length > 0 ? (compliantRules.length / relevantRules.length) * 100 : 100;
  ...
  async getAuditLog(tableName?: string, action?: string, limit: number = 100): Promise<any[]> {
    this.validateConnection();
  ...
  async getStatistics(): Promise<MemoryStatistics> {
    ...
    try {
      ...
      const totalSessions = (totalSessionsResult as { sessions: number }[])[0].sessions;
      ...
      const compliantSessions = (complianceResult as { compliant: number }[])[0].compliant;
