...
describe("i18n Konfiguration", () => {
  ...
  describe("Hero Section Übersetzungen", () => {
    it("should translate hero titel to German", () => {
      expect(i18n.t("hero.titel")).toBe("Digitale Lösungen mit Verstand & Verantwortung");
    });

    it("should translate hero titel to English", async () => {
      await i18n.changeLanguage("en");
      expect(i18n.t("hero.titel")).toBe("Digital Solutions with Intelligence & Care");
    });

    it("should translate hero titel to Spanish", async () => {
      await i18n.changeLanguage("es");
      expect(i18n.t("hero.titel")).toBe("Soluciones Digitales con Inteligencia y Cuidado");
    });

    it("should translate hero untertitel to German", () => {
      expect(i18n.t("hero.untertitel")).toBe("Barrierefrei. Persönlich. Automatisiert.");
    });

    it("should translate hero untertitel to English", async () => {
      await i18n.changeLanguage("en");
      expect(i18n.t("hero.untertitel")).toBe("Accessible. Personal. Automated.");
    });

    it("should translate hero untertitel to Spanish", async () => {
      await i18n.changeLanguage("es");
      expect(i18n.t("hero.untertitel")).toBe("Accesible. Personal. Automatizado.");
    });
  });
  ...
  describe("Hauptbereiche Section Übersetzungen", () => {
    ...
    it("should translate all service descriptions to German", () => {
      serviceKeys.forEach((key) => {
        const translation = i18n.t(`hauptbereiche.elemente.${key}.beschreibung`);
        expect(translation).toBeTruthy();
        expect(typeof translation).toBe("string");
      });
    });
  ...
  describe("Webshop Section Übersetzungen", () => {
    it("should translate webshop titel to German", () => {
      expect(i18n.t("webshop.titel")).toBe("Unser Webshop – Digital. Einfach. Sofort.");
    });

    it("should translate webshop titel to English", async () => {
      await i18n.changeLanguage("en");
      expect(i18n.t("webshop.titel")).toBe("Our Shop – Digital. Simple. Instant.");
    });

    it("should translate webshop titel to Spanish", async () => {
      await i18n.changeLanguage("es");
      expect(i18n.t("webshop.titel")).toBe("Nuestra Tienda – Digital. Simple. Instantáneo.");
    });
  });
