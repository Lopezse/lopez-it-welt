...
const RuleEnforcementContext = createContext<RuleEnforcementContextType | null>(null);

export function useRuleEnforcement() {
  const context = useContext(RuleEnforcementContext);
  if (!context) {
    throw new Error("useRuleEnforcement must be used within RuleEnforcementProvider");
  }
  return context;
}
...
export function RuleEnforcementProvider({ children }: RuleEnforcementProviderProps) {
  ...
  // Verstoß-Erkennung
  const checkForViolation = (action: string, rules: QualityRules): string | null => {
  ...
  }, [isBlocked, violationCount, lastViolation, currentAction, pendingApproval, approveAction]);
        ...
        <div className="fixed inset-0 bg-red-900 bg-opacity-90 z-50 flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg shadow-2xl max-w-2xl mx-4">
            <div className="text-center">
              <h1 className="text-3xl font-bold text-red-600 mb-4">🚨 KI-SYSTEM BLOCKIERT</h1>

              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <h2 className="text-xl font-semibold text-red-800 mb-2">Regelverstoß erkannt</h2>
              ...
              <div className="mt-6 text-sm text-gray-600">
                <p>Das KI-System ist blockiert, bis Sie explizit eine Freigabe erteilen.</p>
