...
export async function generateKundennummer(): Promise<string> {
  try {
    ...
    const [rows] = await connection.execute(
      'SELECT MAX(CAST(SUBSTRING(kundennummer, 5) AS UNSIGNED)) as max_num FROM lopez_customers WHERE kundennummer LIKE "KUN-%"',
    );

    const maxNum = (rows as any)[0]?.max_num || 0;
    const nextNum = maxNum + 1;

    return `KUN-${String(nextNum).padStart(6, "0")}`;
  } catch (error) {
    console.error("❌ Kundennummer-Generierung fehlgeschlagen:", error);
    return `KUN-${Date.now().toString().slice(-6)}`;
  }
}
