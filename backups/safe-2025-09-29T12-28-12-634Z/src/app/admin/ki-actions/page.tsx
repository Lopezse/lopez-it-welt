      ...
      <div className="space-y-4">
        {sessions.map((session) => (
          <Card key={session.session_id}>
            <div className="mb-3">
              <h3 className="text-lg font-semibold text-gray-900">{session.modul}</h3>
              <p className="text-sm text-gray-600">{session.session_id}</p>
            </div>

            <div className="mb-3">
              <p className="text-sm text-gray-700 mb-2">
                <span className="font-medium">Trigger:</span> {session.triggered_by}
              </p>
              <p className="text-sm text-gray-700 mb-2">
                <span className="font-medium">Zusammenfassung:</span> {session.summary}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
              <div>
                <span className="font-medium">Start:</span> {formatDateTime(session.start_time)}
              </div>
              {session.end_time && (
                <div>
                  <span className="font-medium">Ende:</span> {formatDateTime(session.end_time)}
                </div>
              )}
              {session.duration_minutes && (
                <div>
                  <span className="font-medium">Dauer:</span> {session.duration_minutes} Min
                </div>
              )}
            </div>
          </Card>
