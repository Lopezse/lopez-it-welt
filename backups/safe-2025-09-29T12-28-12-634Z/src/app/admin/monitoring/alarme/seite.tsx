...
export default function AlarmSeite() {
  ...
  const [filter, setFilter] = useState<"alle" | "aktiv" | "kritisch" | "nicht_bestaetigt">("alle");
    ...
    const mockAlarme: Alarm[] = [
      {
        ...
        beschreibung: "Mehrere fehlgeschlagene Login-Versuche von verdächtiger IP-Adresse erkannt",
  ...
  const alarmBestaetigen = (id: string) => {
    setAlarme((prev) =>
      prev.map((alarm) => (alarm.id === id ? { ...alarm, status: "bestätigt" as const } : alarm)),
    );
  };

  const alarmLoesen = (id: string) => {
    setAlarme((prev) =>
      prev.map((alarm) => (alarm.id === id ? { ...alarm, status: "gelöst" as const } : alarm)),
    );
  };
      ...
      <div className="relative z-10 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="backdrop-blur-sm bg-weiss/10 rounded-xl p-4 border border-weiss/20">
                ...
                <p className="text-2xl font-bold text-rot">{statistiken.aktiv}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="backdrop-blur-xl bg-weiss/10 rounded-2xl border border-weiss/20 p-6 shadow-2xl text-center">
            <div className="text-3xl font-bold text-rot mb-2">{statistiken.kritisch}</div>
            <div className="text-sm text-hellgrau">Kritisch</div>
          </div>
          <div className="backdrop-blur-xl bg-weiss/10 rounded-2xl border border-weiss/20 p-6 shadow-2xl text-center">
            <div className="text-3xl font-bold text-orange mb-2">{statistiken.hoch}</div>
            <div className="text-sm text-hellgrau">Hoch</div>
          </div>
          <div className="backdrop-blur-xl bg-weiss/10 rounded-2xl border border-weiss/20 p-6 shadow-2xl text-center">
            <div className="text-3xl font-bold text-gelb mb-2">{statistiken.mittel}</div>
            <div className="text-sm text-hellgrau">Mittel</div>
          </div>
          <div className="backdrop-blur-xl bg-weiss/10 rounded-2xl border border-weiss/20 p-6 shadow-2xl text-center">
            <div className="text-3xl font-bold text-akzentblau mb-2">{statistiken.niedrig}</div>
            ...
            <div className="flex space-x-2">
              {[
                ...
                <button
                  ...
                  onClick={() =>
                    setFilter(
                      filterOption.key as "alle" | "aktiv" | "kritisch" | "nicht_bestaetigt",
                    )
                  }
                    ...
                    <div className="flex items-start space-x-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <h3 className="text-lg font-semibold text-weiss">{alarm.titel}</h3>
                          <span
                        ...
                        <p className="text-hellgrau mb-3">{alarm.beschreibung}</p>
