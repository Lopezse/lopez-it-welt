...
export async function GET(request: NextRequest) {
  try {
    ...
    // Customers API Fehler: ${error}
    return NextResponse.json(
      { success: false, message: "Fehler beim Laden der Kunden" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const customerId = await createCustomer({
      ...
      company_name,
      ust_id,
      contact_person_anrede: contact_person_anrede || "Keine Angabe",
      contact_person_titel,
      contact_person_vorname,
      contact_person_nachname,
      email,
      ...
      land_iso: land_iso || "DE",
      support_level: support_level || "Standard",
      account_manager,
      status: status || "aktiv",
      notes,
      created_by: "admin_user", // In Produktion: aus JWT-Token
    });
    ...
    // Kunde erstellen Fehler: ${error}
    return NextResponse.json(
      { success: false, message: "Fehler beim Erstellen des Kunden" },
      { status: 500 },
    );
  }
}
...
async function createCustomer(customerData: any) {
  // Demo: Customer-ID generieren (in Produktion: Datenbank-Insert)
  const customerId = `customer_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;

  // Neuer Kunde erstellt: ${customerId}
