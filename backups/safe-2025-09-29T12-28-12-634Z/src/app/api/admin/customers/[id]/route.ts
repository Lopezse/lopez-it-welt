...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const customerId = params.id;
    const { action = "deactivate" } = await request.json();

    if (action === "delete") {
      // Kunde permanent löschen (nur mit besonderen Rechten)
      ...
      // Kunde deaktivieren
      const deactivated = await deactivateCustomer(customerId);
    ...
    // Customer Delete Fehler: ${error}
    return NextResponse.json(
      {
        success: false,
        message: "Fehler beim Löschen/Deaktivieren des Kunden",
      },
      { status: 500 },
    );
  }
