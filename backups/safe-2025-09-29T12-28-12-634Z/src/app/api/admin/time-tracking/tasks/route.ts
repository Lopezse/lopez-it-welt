...
export async function POST(request: NextRequest) {
  try {
    ...
    // Neue Task erstellt: ${newTask.title}
    return NextResponse.json(newTask, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: "Fehler beim Erstellen der Task" }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Fehler beim Aktualisieren der Task" }, { status: 500 });
  }
}
