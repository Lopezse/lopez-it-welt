...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      // JSON Parse Fehler: ${jsonError}
      body = {};
    }

    const { end_time, status = "completed", bemerkung, lektion, ursache, naechster_schritt } = body;
    ...
    // Fehler beim Beenden der Session: ${error}
    return NextResponse.json({ error: "Fehler beim Beenden der Session" }, { status: 500 });
  }
}
