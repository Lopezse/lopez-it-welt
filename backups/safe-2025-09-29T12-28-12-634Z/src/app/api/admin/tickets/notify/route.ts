...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validierung
    if (!notification.ticket_id || !notification.customer_id || !notification.subject) {
    ...
    // Neues Ticket vom Shop erhalten: ${notification.ticket_id}
    ...
    const recipients = priorityEmails[notification.priority] || ["support@lopez-team.de"];
    ...
    recipients.forEach((email) => {
      ...
      // E-Mail-Benachrichtigung an ${email}: ${emailNotification.subject}
    });
    ...
    const category = getTicketCategory(notification.ticket_type, notification.subject);
    ...
    // Fehler bei Ticket-Benachrichtigung: ${error}
...
function getTicketCategory(type: string, subject: string): string {
  ...
  if (type === "technical") {
    ...
    if (lowerSubject.includes("mobile") || lowerSubject.includes("app")) return "Mobile-Probleme";
    return "Technische Probleme";
  }

  if (type === "payment") {
    if (lowerSubject.includes("rückerstattung") || lowerSubject.includes("refund"))
  ...
  if (type === "shipping") {
    ...
    if (lowerSubject.includes("verloren") || lowerSubject.includes("lost")) return "Paketverlust";
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fehler beim Abrufen des Ticket-Status: ${error}
    return NextResponse.json({ error: "Interner Server-Fehler" }, { status: 500 });
  }
}
