...
export class DSGVOEnforcementSystem {
    ...
    for (const pattern of sensitiveDataPatterns) {
      if (pattern.pattern.test(input)) {
        violations.push({
          ...
          suggestion:
            'Sensible Daten sollten nicht in Chat-Inputs verwendet werden',
    ...
    const dsgvoViolations = [
      {
        ...
        suggestion:
          'Vermeiden Sie die Verarbeitung personenbezogener Daten ohne Rechtsgrundlage',
      },
      {
    ...
    for (const violation of dsgvoViolations) {
      if (response.includes(violation.keyword)) {
        violations.push({
          ...
          severity: violation.severity as
            | 'CRITICAL'
            | 'HIGH'
            | 'MEDIUM'
            | 'LOW',
  ...
  private generateComplianceSuggestions(
    ...
    if (violationTypes.includes('DATA_COLLECTION')) {
      ...
      suggestions.push(
        '⚖️ Rechtsgrundlagen für Datenverarbeitung dokumentieren'
      );
    }

    if (violationTypes.includes('USER_RIGHTS')) {
      suggestions.push(
        '👤 Betroffenenrechte (Auskunft, Löschung) implementieren'
      );
      suggestions.push(
        '📧 Kontaktmöglichkeit für Datenschutzanfragen einrichten'
      );
    }
    ...
    if (suggestions.length === 0) {
      suggestions.push(
        '✅ DSGVO-Compliance ist gut - regelmäßige Überprüfung empfohlen'
      );
    }
