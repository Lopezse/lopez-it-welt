...
export class QualityAgent extends KIAgent {
  ...
  private calculateComplexity(content: string): number {
    ...
    complexity +=
      ifStatements + forLoops + whileLoops + switchStatements + catchBlocks;
  ...
  private async identifyIssues(
    ...
    if (content.includes('password') || content.includes('secret')) {
      issues.push(
        '❌ Passwort/Secret im Code gefunden - Environment Variables verwenden'
      );
    }
    ...
    if (
      ...
      issues.push(
        '❌ Englische Komponenten-Namen gefunden - Deutsche Namen verwenden'
      );
    }
  ...
  private async generateRecommendations(
    ...
    if (issues.some(issue => issue.includes('any'))) {
      recommendations.push(
        '✅ any-Typen durch spezifische Interfaces ersetzen'
      );
    }
    ...
    if (metrics.duplication > 5) {
      recommendations.push(
        '✅ Code-Duplikation reduzieren - Gemeinsame Funktionen extrahieren'
      );
    }
  ...
  async generateTestFile(filePath: string): Promise<string> {
    try {
      ...
      const content = fs.readFileSync(filePath, 'utf8');
      const fileName =
        filePath.split('/').pop()?.replace('.tsx', '').replace('.ts', '') ||
        'Component';
