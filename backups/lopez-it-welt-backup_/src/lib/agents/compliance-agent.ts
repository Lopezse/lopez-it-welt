...
export class ComplianceAgent extends KIAgent {
  ...
  async createDSGVOCompliantForm(formData: DSGVOFormData): Promise<{
    ...
    try {
      ...
      const rules = await this.memory.recallRules(
        'DSGVO Formular Consent Datenschutz',
        5
      );
  ...
  private async validateDSGVOCompliance(
    ...
    if (
      !formData.fields.some(field => field.toLowerCase().includes('consent'))
    ) {
    ...
    if (
      !formData.fields.some(field =>
        field.toLowerCase().includes('datenschutz')
      )
    ) {
...
export default ${formData.formName};
  ...
  private determineFieldType(fieldName: string): string {
    ...
    if (lowerField.includes('email') || lowerField.includes('e-mail'))
      return 'email';
    if (lowerField.includes('phone') || lowerField.includes('telefon'))
      return 'phone';
    if (
  ...
  private async validateFinalForm(
    form: string,
    rules: string[]
  ): Promise<ComplianceResult> {
    const violations: string[] = [];
    ...
    try {
      ...
      if (!formContent.includes('required')) {
        ...
        recommendations.push(
          '✅ Required-Attribute für Pflichtfelder hinzufügen'
        );
      }
