...
export class MemoryIntegrationAgent extends KIAgent {
  ...
  private async validateCompliance(task: KITask): Promise<KITaskResult> {
    try {
      ...
      return this.createFailedResult(
        `Compliance-Prüfung fehlgeschlagen: ${error}`
      );
    }
  }
  ...
  private async storeSession(task: KITask): Promise<KITaskResult> {
    try {
      ...
      return this.createFailedResult(
        `Session speichern fehlgeschlagen: ${error}`
      );
    }
  }
  ...
  private async getStatistics(task: KITask): Promise<KITaskResult> {
    try {
      ...
      return this.createFailedResult(
        `Statistiken abrufen fehlgeschlagen: ${error}`
      );
    }
  }
  ...
  private async testIntegration(task: KITask): Promise<KITaskResult> {
    try {
      ...
      const complianceRules =
        await this.memorySystem.getRulesForAgent('compliance');
      results.push({
      ...
      const enterpriseRules =
        await this.memorySystem.getRulesForAgent('enterprise');
      results.push({
      ...
      return this.createFailedResult(
        `Integration-Test fehlgeschlagen: ${error}`
      );
    }
  }
  ...
  async validateAction(
    ...
    return await this.memorySystem.validateAgentAction(
      agentType,
      action,
      context
    );
