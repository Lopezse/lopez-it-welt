...
class BusinessReportGenerator {
  ...
  static async generateBusinessOverview(
    ...
    const revenueAnalytics = await this.collectRevenueAnalytics(
      startDate,
      endDate
    );
...
class SystemPerformanceGenerator {
  ...
  static async generateSystemPerformance(
    ...
    return {
      ...
      databasePerformance: await this.collectDatabasePerformance(
        startDate,
        endDate
      ),
...
class ReportGenerator {
  ...
  private static async collectReportData(
    ...
    switch (reportConfig.metadata.type) {
      ...
      default:
        throw new Error(
          `Unbekannter Report-Typ: ${reportConfig.metadata.type}`
        );
    }
  }
