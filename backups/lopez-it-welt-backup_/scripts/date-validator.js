...
class DateValidator {
  ...
  async validateAllDates() {
    ...
    console.log(
      `📅 Aktuelles Datum: ${this.currentDateGerman} (${this.currentDateString})\n`
    );
    ...
    for (const file of configFiles) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          if (
            content.includes('2025-06-27') ||
            content.includes('27.06.2025')
          ) {
  ...
  generateReport() {
    ...
    if (this.errors.length === 0) {
      ...
      console.log(
        '   - Verwenden Sie das aktuelle Datum: ' + this.currentDateGerman
      );
