...
class MdDateiAnalysator {
    ...
    extrahiereKategorien(inhalt) {
        ...
        if (inhalt.includes('Admin') || inhalt.includes('Dashboard') || inhalt.includes('Zeiterfassung')) {
        ...
        if (inhalt.includes('Content-Management') || inhalt.includes('CMS') || inhalt.includes('Webseite') || inhalt.includes('Text') || inhalt.includes('Dynamisch')) {
    ...
    extrahiereSchluesselwoerter(inhalt) {
        const woerter = inhalt.toLowerCase()
            .replace(/[^\w\s]/g, ' ')
            .split(/\s+/)
            .filter(wort => wort.length > 3)
            .filter(wort => !['und', 'der', 'die', 'das', 'den', 'dem', 'des', 'eine', 'einer', 'eines', 'mit', 'von', 'für', 'auf', 'in', 'an', 'bei', 'seit', 'nach', 'vor', 'über', 'unter', 'zwischen', 'durch', 'ohne', 'gegen', 'um', 'wegen', 'trotz', 'während', 'innerhalb', 'außerhalb'].includes(wort));
...
class ComplianceAgent {
    ...
    extrahiereRegelnUndGesetze(dateien) {
        ...
        dateien.forEach(datei => {
            ...
            if (inhalt.includes('regel') || inhalt.includes('richtlinie') || inhalt.includes('vorschrift') || inhalt.includes('standard')) {
                const regelMatches = inhalt.match(/(?:regel|richtlinie|vorschrift|standard)[:\s]*([^.\n]+)/gi);
                if (regelMatches) {
            ...
            if (inhalt.includes('id') || inhalt.includes('identifier') || inhalt.includes('namenskonvention')) {
            ...
            if (inhalt.includes('klasse') || inhalt.includes('class') || inhalt.includes('struktur') || inhalt.includes('architektur')) {
                const klassenMatches = inhalt.match(/(?:klasse|class|struktur|architektur)[:\s]*([^.\n]+)/gi);
                if (klassenMatches) {
            ...
            if (inhalt.includes('ki') || inhalt.includes('ai') || inhalt.includes('intelligence') || inhalt.includes('agent') || inhalt.includes('automation')) {
            ...
            // Gesetze extrahieren
            if (inhalt.includes('gesetz') || inhalt.includes('dsgvo') || inhalt.includes('gdpr') || inhalt.includes('compliance') || inhalt.includes('recht') || inhalt.includes('legal')) {
                const gesetzMatches = inhalt.match(/(?:gesetz|dsgvo|gdpr|compliance|recht|legal)[:\s]*([^.\n]+)/gi);
                if (gesetzMatches) {
            ...
            if (inhalt.includes('qualität') || inhalt.includes('quality') || inhalt.includes('standard')) {
            ...
            if (inhalt.includes('sicherheit') || inhalt.includes('security') || inhalt.includes('schutz')) {
            ...
            if (inhalt.includes('enterprise') || inhalt.includes('business') || inhalt.includes('unternehmen')) {
                const enterpriseMatches = inhalt.match(/(?:enterprise|business|unternehmen)[:\s]*([^.\n]+)/gi);
    ...
    bestimmeRegelPrioritaet(regel) {
        ...
        if (regelText.includes('kritisch') || regelText.includes('wichtig') || regelText.includes('muss') || regelText.includes('pflicht')) {
            return 'hoch';
        } else if (regelText.includes('soll') || regelText.includes('empfohlen') || regelText.includes('best practice')) {
            return 'normal';
    ...
    pruefeCompliance(aktuelleAktivitaet) {
        ...
        this.complianceStatus = verstoesse.length > 0 ? 'verstoß' : warnungen.length > 0 ? 'warnung' : 'konform';
    ...
    pruefeRegelCompliance(regel, aktivitaet) {
        ...
        if (regel.typ === 'id') {
            if (regelText.includes('camelcase') && /[A-Z][a-z]/.test(aktivitaetText)) {
                return { warnung: 'CamelCase-Namenskonvention möglicherweise nicht eingehalten' };
            }
            if (regelText.includes('snake_case') && /_/.test(aktivitaetText)) {
                return { warnung: 'Snake_case-Namenskonvention möglicherweise nicht eingehalten' };
            }
        }
        ...
        if (regel.typ === 'ki') {
            if (regelText.includes('agent') && !aktivitaetText.includes('agent')) {
                return { warnung: 'KI-Agenten-Richtlinien möglicherweise nicht eingehalten' };
            }
            if (regelText.includes('gedächtnis') && !aktivitaetText.includes('gedächtnis')) {
                return { warnung: 'KI-Gedächtnis-Richtlinien möglicherweise nicht eingehalten' };
            }
        }
        ...
        if (regel.typ === 'qualitaet') {
            if (regelText.includes('qualität') && aktivitaetText.includes('fehler')) {
                return { warnung: 'Qualitätsstandards möglicherweise nicht eingehalten' };
            }
        }
        ...
        if (regel.typ === 'enterprise') {
            if (regelText.includes('enterprise') && aktivitaetText.includes('test')) {
                return { warnung: 'Enterprise-Standards möglicherweise nicht eingehalten' };
            }
        }
    ...
    pruefeGesetzCompliance(gesetz, aktivitaet) {
        ...
        if (gesetzText.includes('dsgvo') || gesetzText.includes('gdpr')) {
            if (aktivitaetText.includes('personendaten') && !aktivitaetText.includes('einwilligung')) {
                return { verstoß: 'DSGVO: Personendaten ohne Einwilligung verarbeitet' };
            }
        }
...
class AgentenSystem {
    ...
    async fuehreSystemAnalyseDurch() {
        ...
        const { regeln, gesetze, kiRichtlinien, idRegeln, klassenRegeln } = this.complianceAgent.extrahiereRegelnUndGesetze(gedaechtnis.analysierteDateien);
    ...
    identifiziereOffeneAufgaben() {
        ...
        function identifiziereAufgaben(dateien) {
            ...
            dateien.forEach(datei => {
                ...
                if (kategorien.includes('enterprise')) {
                    ...
                    if (inhalt.includes('backup') && inhalt.includes('disaster') && inhalt.includes('recovery')) {
                        aufgaben.push({
        ...
        generiereSystemStatus() {
            ...
            const status = {
                ...
                naechsteSchritte: this.bestimmeNaechsteSchritte(offeneAufgaben)
            };
        ...
        bestimmeNaechsteSchritte(offeneAufgaben) {
            ...
            const hochPrioritaet = offeneAufgaben.filter(a => a.prioritaet === 'hoch');
            ...
            schritte.push({
                ...
                aufgaben: [{ typ: 'system_entwicklung', beschreibung: 'Automatisierte Agenten-Logik implementieren' }]
            });
        ...
        fuehreComplianceCheckDurch() {
            ...
            if (gedaechtnis.analysierteDateien.length > 0) {
                const regelnUndGesetze = complianceAgent.extrahiereRegelnUndGesetze(gedaechtnis.analysierteDateien);
