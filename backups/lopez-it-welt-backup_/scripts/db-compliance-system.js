...
class DBComplianceSystem {
  ...
  async loadActiveRules() {
    try {
      ...
      console.log(
        `📋 Aktive Regeln geladen: ${policies.length} Policies, ${laws.length} Laws, ${ci_rules.length} CI-Rules`
      );
    } catch (error) {
  ...
  async logAgentActivity(
    ...
    try {
      await this.connection.execute('CALL LogAgentActivity(?, ?, ?, ?, ?, ?)', [
        agentName,
        activityType,
        description,
        status,
      ...
      console.error(
        '❌ Fehler beim Protokollieren der Agent-Aktivität:',
        error.message
      );
    }
  }
  ...
  async checkLanguageCompliance(content, context = '') {
    ...
    for (const policy of this.activeRules.policies) {
      if (
        policy.category === 'QUALITÄT' ||
        policy.content.toLowerCase().includes('sprache')
      ) {
        const isGerman =
          /[äöüßÄÖÜ]/.test(content) ||
  ...
  async checkNamingCompliance(code, context = '') {
    ...
    for (const rule of this.activeRules.ci_rules) {
      if (
        rule.category === 'CODE_QUALITY' ||
        rule.content.toLowerCase().includes('naming')
      ) {
        ...
        const variables =
          code.match(/\b(const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g) || [];
        const functions =
          code.match(/\bfunction\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g) || [];
        ...
        const message = namingCompliance
          ? 'Namenskonventionen eingehalten'
          : 'Namenskonventionen verletzt';
  ...
  async checkSecurityCompliance(code, context = '') {
    ...
    for (const rule of this.activeRules.ci_rules) {
      if (
        ...
        rule.content.toLowerCase().includes('sicherheit')
      ) {
  ...
  async checkQualityCompliance(code, context = '') {
    ...
    for (const rule of this.activeRules.ci_rules) {
      if (
        rule.category === 'CODE_QUALITY' ||
        rule.content.toLowerCase().includes('qualität')
      ) {
        const qualityIssues = [];

        // Kommentare prüfen
        const commentRatio = (code.match(/\/\*[\s\S]*?\*\/|\/\/.*$/gm) || [])
          .length;
  ...
  async runFullComplianceCheck(content, code = '', context = '') {
    ...
    try {
      ...
      const languageChecks = await this.checkLanguageCompliance(
        content,
        context
      );
      ...
      this.complianceStatus.passed = allChecks.filter(
        c => c.status === 'PASS'
      ).length;
      this.complianceStatus.failed = allChecks.filter(
        c => c.status === 'FAIL'
      ).length;
      this.complianceStatus.warnings = allChecks.filter(
        c => c.status === 'WARNING'
      ).length;
  ...
  async printComplianceReport(checks) {
    ...
    try {
      ...
      rows.forEach(row => {
        console.log(
          `${row.rule_type}: ${row.active_rules}/${row.total_rules} aktiv`
        );
      });
    } catch (error) {
      console.error(
...
async function main() {
  ...
  await complianceSystem.runFullComplianceCheck(
    testContent,
    testCode,
    'Test-Kontext'
  );
