...
class MemoryIntegrationTest {
  ...
  async testSchemaImport() {
    ...
    try {
      ...
      if (missingColumns.length === 0) {
        ...
        this.testResults.push({
          test: 'Schema importieren',
          success: false,
          error: `Fehlende Spalten: ${missingColumns.join(', ')}`,
        });
      }
    } catch (error) {
  ...
  async testRulesLoading() {
    ...
    try {
      ...
      const totalRules =
        dsgvoRules[0].count +
        enterpriseRules[0].count +
        qualityRules[0].count +
        securityRules[0].count;
  ...
  async testAgentIntegration() {
    ...
    try {
      ...
      complianceRules.forEach(rule => {
        log(
          `  - ${rule.rule_text.substring(0, 60)}... (${rule.priority})`,
          'yellow'
        );
      });
      ...
      if (allAgentsHaveRules) {
        ...
        this.testResults.push({
          test: 'Agenten verbinden - Memory-Integration',
          success: false,
          error: 'Nicht alle Agenten haben Regeln',
        });
      }
    } catch (error) {
      log(
  ...
  async testComplianceValidation() {
    ...
    try {
      ...
      log(
        `🔍 Enterprise++ Compliance: ${enterpriseTest.length} relevante Regeln`,
        'blue'
      );
      log(
        `🔍 Quality-Compliance: ${qualityTest.length} relevante Regeln`,
        'blue'
      );
      log(
        `🔍 Security-Compliance: ${securityTest.length} relevante Regeln`,
        'blue'
      );

      const totalComplianceRules =
        dsgvoTest.length +
        enterpriseTest.length +
        qualityTest.length +
        securityTest.length;
      ...
      if (totalComplianceRules > 0) {
        ...
        this.testResults.push({
          test: 'Compliance-Prüfung',
          success: false,
          error: 'Keine Compliance-Regeln gefunden',
        });
      }
    } catch (error) {
  ...
  async testMemorySessions() {
    ...
    try {
      ...
      if (sessions.length > 0) {
        ...
        log(
          `📝 Compliance-Status: ${sessions[0].compliance_status ? '✅' : '❌'}`,
          'blue'
        );
    ...
    try {
      ...
      if (auditLogs.length > 0) {
        ...
        this.testResults.push({
          test: 'Audit-Log',
          success: false,
          error: 'Audit-Log konnte nicht abgerufen werden',
        });
      }
    } catch (error) {
  ...
  showTestResults() {
    ...
    log(
      `❌ Fehlgeschlagene Tests: ${failedTests}`,
    ...
    if (failedTests === 0) {
      log(
        '\n🎉 Alle Tests erfolgreich! Memory-System Integration funktioniert.',
        'green'
      );
    } else {
