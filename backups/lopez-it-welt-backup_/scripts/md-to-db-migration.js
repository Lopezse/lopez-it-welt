...
class MDMigrationSystem {
  ...
  determineCategory(filePath, content) {
    ...
    if (
      ...
      contentLower.includes('datenschutz')
    ) {
    ...
    if (
      fileName.includes('sicherheit') ||
      contentLower.includes('sicherheit')
    ) {
      return { type: 'POLICY', category: 'SICHERHEIT' };
    }
    if (
      fileName.includes('entwicklung') ||
      contentLower.includes('entwicklung')
    ) {
  ...
  determinePriority(content) {
    ...
    if (
      contentLower.includes('kritisch') ||
  ...
  async insertRule(ruleData) {
    const { type, category, key_title, content, source_file, priority } =
      ruleData;
  ...
  async processMarkdownFile(filePath) {
    try {
      ...
      console.error(
        `❌ Fehler beim Verarbeiten von ${filePath}:`,
        error.message
      );
      this.migrationStats.errors++;
    }
  ...
  async printMigrationReport() {
    ...
    console.log(
      `✅ Verarbeitete Dateien: ${this.migrationStats.processedFiles}`
    );
