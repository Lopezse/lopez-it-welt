...
class MemorySystemIntegration {
  ...
  async connect() {
    try {
      ...
      log(
        `❌ MySQL Memory System Verbindung fehlgeschlagen: ${error.message}`,
        'red'
      );
      throw error;
    }
  ...
  async loadDefaultRules() {
    ...
    try {
      ...
      const dsgvoRules = [
        ...
        {
          rule_text:
            'Opt-Out-Mechanismus für Marketing-Cookies muss vorhanden sein',
          category: 'compliance',
      ...
      const enterpriseRules = [
        ...
        {
          rule_text:
            'Zero Tolerance Policy: Keine Regelverstöße werden toleriert',
          category: 'enterprise',
        ...
        {
          rule_text:
            'Strict Mode Enforcement: Alle Systeme im strengsten Modus',
          category: 'enterprise',
      ...
      log(
        `❌ Laden der Standard-Regeln fehlgeschlagen: ${error.message}`,
        'red'
      );
      throw error;
    }
  ...
  async testAgentIntegration() {
    ...
    try {
      ...
      log(
        `✅ Compliance-Agent: ${complianceRules[0].length} Regeln verfügbar`,
        'green'
      );
      ...
      log(
        `✅ Enterprise-Agent: ${enterpriseRules[0].length} Regeln verfügbar`,
        'green'
      );
      ...
      log(
        `✅ Quality-Agent: ${qualityRules[0].length} Regeln verfügbar`,
        'green'
      );
      ...
      log(
        `✅ Security-Agent: ${securityRules[0].length} Regeln verfügbar`,
        'green'
      );
    } catch (error) {
      log(
        `❌ Agenten-Integration-Test fehlgeschlagen: ${error.message}`,
        'red'
      );
      throw error;
    }
      ...
      for (const test of testActions) {
        ...
        const hasViolations = relevantRules.some(rule => {
          ...
          if (rule.category === 'compliance' && ruleLower.includes('dsgvo')) {
            return (
              actionLower.includes('formular') &&
              !actionLower.includes('consent')
            );
          }

          if (rule.category === 'enterprise' && ruleLower.includes('test')) {
            return (
              actionLower.includes('test') && !actionLower.includes('coverage')
            );
          }
  ...
  async showStatistics() {
    ...
    try {
      ...
      log(
        `❌ Statistiken konnten nicht abgerufen werden: ${error.message}`,
        'red'
      );
    }
  }
  ...
  async run() {
    try {
      ...
      log(
        `💥 Memory-System Integration fehlgeschlagen: ${error.message}`,
        'red'
      );
      process.exit(1);
    } finally {
