...
class FlowBlocker {
  ...
  async checkFlow(action = 'unknown') {
    ...
    console.log(
      `🔍 FLOW CHECK: Schritt ${this.stepCounter}/${this.maxStepsBeforeCheck}`
    );
    ...
    if (
      this.stepCounter >= this.maxStepsBeforeCheck ||
      timeSinceLastCheck >= twoHours
    ) {
      ...
      console.log(
        '✅ FLOW CHECK: Alle Regeln eingehalten - Weiterarbeit erlaubt'
      );
    }
  ...
  checkQualityCompliance() {
    ...
    if (!fs.existsSync(qualityPath)) {
      violations.push({
        ...
        severity: 'CRITICAL',
        message:
          'QualityController.md fehlt - Qualitätsstandards nicht definiert',
      });
      return violations;
    }
  ...
  async blockFlow(violations, action) {
    ...
    console.log('');
    console.log(
      '⏸️ FLOW PAUSIERT - Bitte beheben Sie die Verletzungen und geben Sie "GO" ein'
    );
    console.log('');
    console.log('💡 Tipps:');
    console.log('  - Prüfen Sie STATUS.md für Details');
    console.log(
      '  - Führen Sie node scripts/enforce-rules.js auto-correct aus'
    );
    console.log(
...
${statusContent}`;
  ...
  showStatus() {
    ...
    console.log(
      `  - Letzte Prüfung: ${new Date(this.lastStatusCheck).toLocaleString('de-DE')}`
    );
    console.log(
      `  - Nächste Prüfung: ${this.maxStepsBeforeCheck - this.stepCounter} Schritte`
    );
    ...
    if (timeUntilNextCheck > 0) {
      ...
      const minutes = Math.floor(
        (timeUntilNextCheck % (1000 * 60 * 60)) / (1000 * 60)
      );
