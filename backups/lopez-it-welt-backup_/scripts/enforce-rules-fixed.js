...
class EnterpriseRuleEnforcement {
  ...
  async checkRequiredFiles() {
    ...
    for (const file of requiredFiles) {
      ...
      if (!fs.existsSync(filePath)) {
        this.violations.push(`Pflichtdatei fehlt: ${file}`);
      ...
      if (dateMatches) {
        dateMatches.forEach(dateStr => {
          ...
          const daysDiff = Math.floor(
            (this.currentDate - fileDate) / (1000 * 60 * 60 * 24)
          );
  ...
  async checkStructureIntegrity() {
    ...
    const unauthorizedFiles = allMdFiles.filter(
      file => !allowedMdFiles.includes(file)
    );
      ...
      try {
        ...
        if (activeSessions.length > 1) {
          this.violations.push(
            `Überlappende Sessions: ${activeSessions.length} aktive Sessions`
          );
