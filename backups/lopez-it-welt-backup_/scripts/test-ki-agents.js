...
const {
  TextManagementAgent,
} = require('../src/lib/agents/text-management-agent.ts');
...
async function testKIAgents() {
  ...
  try {
    ...
    const complianceResult =
      await complianceAgent.createDSGVOCompliantForm(formData);
    ...
    const textKeys = await textAgent.generateTextKeys(
      'src/components/Core/Header.tsx'
    );
    console.log(
      `✅ Text-Management-Agent: ${textKeys.length} Text-Keys extrahiert`
    );
    ...
    const qualityReport = await qualityAgent.analyzeCodeQuality(
      'src/components/Core/Header.tsx'
    );
    ...
    const integratedResult = await testAgentIntegration(
      complianceAgent,
      qualityAgent
    );
...
async function testAgentIntegration(complianceAgent, qualityAgent) {
  try {
    ...
    const qualityReport =
      await qualityAgent.analyzeCodeQuality('generated-form.tsx');
